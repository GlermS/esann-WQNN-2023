...
def exec(config):
    epsilon, decay_rate, learning_rate, forget , tuple_size= config
    input_size = len(encode_func({'mghi': 0, 'zenith':0, 'azimuth':0, 'angle':0}))

    path = f'results/{inputs}/{search_mode}/{model_type}/epsilon={epsilon}_decay-rate={decay_rate}_learning-rate={learning_rate}_forget-factor={forget}_tuple-size={tuple_size}/'
    ...
    run.run_individual(dfCs, ModelGenerator, encode_func, path, greedyRate=epsilon, decay_rate = decay_rate, learning_rate=decay_rate)
        ...
        for decay_rate in setup['params']['decay_rate']:
            for tuple_size in setup['params']['tuple_size']:
                for forget_factor in setup['params']['forget_factor']:
                    for epsilon in setup['search']['epsilon']:
                        configs.append((epsilon, decay_rate, learning_rate, forget_factor, tuple_size))
