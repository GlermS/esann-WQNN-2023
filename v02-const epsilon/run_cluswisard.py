...
def exec(config):
    epsilon, decay_rate, learning_rate, mean_type, tuple_size = config
    path = f'results/{inputs}/{search_mode}/{model_type}/epsilon={epsilon}_decay-rate={decay_rate}_learning-rate={learning_rate}_mean-type={mean_type}_tuple-size={tuple_size}/'
            ...
            for tuple_size in setup['params']['tuple_size']:
                for mean_type in setup['params']['mean_type']:
                    for epsilon in setup['search']['epsilon']:
                        configs.append((epsilon, decay_rate, learning_rate, mean_type, tuple_size))
