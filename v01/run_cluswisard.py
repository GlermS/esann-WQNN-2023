...
def exec(config):
    decay_rate, learning_rate, mean_type, tuple_size = config
    path = f'results/{inputs}/{model_type}/decay-rate={decay_rate}_learning-rate={learning_rate}_mean_type={mean_type}_tuple-size={tuple_size}/'
        ...
        for decay_rate in setup['params']['decay_rate']:
            for tuple_size in setup['params']['tuple_size']:
                for mean_type in setup['params']['mean_type']:
                    configs.append((decay_rate, learning_rate, mean_type, tuple_size))
